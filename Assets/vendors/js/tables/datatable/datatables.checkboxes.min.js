  ...
  Checkboxes.prototype = {
    _constructor: function () {
      ...
      for (var i = 0; i < ctx.aoColumns.length; i++) {
        if (ctx.aoColumns[i].checkboxes) {
          ...
          var colOptions = { searchable: false, orderable: false }
          if (ctx.aoColumns[i].sClass === '') {
            colOptions['className'] = 'dt-checkboxes-cell'
          } else {
            colOptions['className'] =
          ...
          if (ctx.aoColumns[i].checkboxes.selectAll) {
            ...
            if (ctx.aoColumns[i].checkboxes.selectAllRender !== null) {
              ...
              if ($.isFunction(ctx.aoColumns[i].checkboxes.selectAllRender)) {
                ...
                if (
                  typeof ctx.aoColumns[i].checkboxes.selectAllRender ===
                  'string'
                ) {
                  selectAllHtml = ctx.aoColumns[i].checkboxes.selectAllRender
                }
      ...
      if (hasCheckboxes) {
        ...
        if (hasCheckboxesSelectRow) {
          ...
          $table.on(
            'draw.dt.dtCheckboxes select.dt.dtCheckboxes deselect.dt.dtCheckboxes',
            function () {
        ...
        $tableContainer.on(
          'click.dtCheckboxes',
          'thead th.dt-checkboxes-select-all input[type="checkbox"]',
          function (e) {
        ...
        $tableContainer.on(
          'click.dtCheckboxes',
          'thead th.dt-checkboxes-select-all',
          function () {
        ...
        $(document).on(
          'click.dtCheckboxes',
          '.fixedHeader-floating thead th.dt-checkboxes-select-all',
          function () {
            if (ctx._fixedHeader) {
              if (ctx._fixedHeader.dom['header'].floating) {
        ...
        $table.on(
          'stateSaveParams.dt.dtCheckboxes',
      ...
      if (ctx.oFeatures.bStateSave) {
        ...
        $.each(self.s.columns, function (index, colIdx) {
          if (
            state &&
            state.checkboxes &&
            state.checkboxes.hasOwnProperty(colIdx)
          ) {
            if (ctx.aoColumns[colIdx].checkboxes.stateSave) {
    ...
    updateSelectAll: function (colIdx) {
      ...
      if (
        ...
        var $checkboxesSelectAll = $(
          '.dt-checkboxes-select-all[data-col="' +
            colIdx +
            '"] input[type="checkbox"]',
          $tableContainer
        )
        ...
        if (ctx._fixedHeader) {
          if (ctx._fixedHeader.dom['header'].floating) {
            $checkboxesSelectAll = $(
              '.fixedHeader-floating .dt-checkboxes-select-all[data-col="' +
                colIdx +
                '"] input[type="checkbox"]'
            )
          }
        ...
        if (
          ...
          isIndeterminateNow !== isIndeterminate
        ) {
          ...
          if (
            $.isFunction(ctx.aoColumns[colIdx].checkboxes.selectAllCallback)
          ) {
    ...
    getSelectRowColIndex: function () {
      ...
      for (var i = 0; i < ctx.aoColumns.length; i++) {
        if (
          ctx.aoColumns[i].checkboxes &&
          ctx.aoColumns[i].checkboxes.selectRow
        ) {
          colIdx = i
          break
        }
    ...
    updateFixedColumn: function (colIdx) {
      ...
      if (ctx._oFixedColumns) {
        ...
        var rightCols =
          ctx.aoColumns.length - ctx._oFixedColumns.s.iRightColumns - 1
