...
function enterChat(s) {
  ...
  if (/\S/.test(e)) {
    ...
    $('.chat:last-child .chat-body').append(a),
      $('.message').val(''),
...
$(function () {
  ...
  if ($.app.menu.is_touch_device())
    s.css('overflow', 'scroll'),
      n.find('.user-profile-sidebar-area').css('overflow', 'scroll'),
    ...
    if (n.find('.user-profile-sidebar-area').length > 0)
      new PerfectScrollbar(n.find('.user-profile-sidebar-area')[0])
  ...
  if (
    ...
    l.length &&
      l.on('change', function () {
        ...
        e.removeClass(),
          o.find('.avatar span').removeClass(),
          e.addClass(s + ' avatar-status-lg'),
          o.find('.avatar span').addClass(s)
      }),
    d.length &&
      d.on('click', function () {
        e.removeClass('show'),
          n.removeClass('show'),
          sidebarContent.hasClass('show') || overlay.removeClass('show')
      }),
    f.length &&
      f.on('click', function () {
        sidebarContent.removeClass('show'), overlay.removeClass('show')
    ...
    s.find('ul li').length &&
      s.find('ul li').on('click', function () {
        var e = $(this),
          a = $('.start-chat-area'),
          o = $('.active-chat')
        s.find('ul li').hasClass('active') &&
          s.find('ul li').removeClass('active'),
          e.addClass('active'),
          e.find('.badge').remove(),
          s.find('ul li').hasClass('active')
            ? (a.addClass('d-none'), o.removeClass('d-none'))
            : (a.removeClass('d-none'), o.addClass('d-none'))
      }),
    ...
    $(window).width() < 992 && sidebarToggleFunction(),
    w.length &&
      w.on('keyup', function () {
        ...
        if ('' !== e) {
          ...
          0 == a
            ...
            0 == o
              ? h.find('.no-results').addClass('show')
              : h.find('.no-results').hasClass('show') &&
                h.find('.no-results').removeClass('show')
  ...
  $(window).on('resize', function () {
    sidebarToggleFunction(),
      $(window).width() > 992 &&
        $('.chat-application .body-content-overlay').hasClass('show') &&
