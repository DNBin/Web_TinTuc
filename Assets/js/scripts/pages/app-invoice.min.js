$(function () {
  ...
  r.length &&
    ...
    u.length &&
      u.select2({ placeholder: 'Select country', dropdownParent: u.parent() }),
    ...
    p.length &&
      ...
      p.on('change', function () {
        var e = $(this),
          t =
            '<div class="customer-details mt-1"><p class="mb-25">' +
            f[e.val()].name +
            '</p><p class="mb-25">' +
            f[e.val()].company +
            '</p><p class="mb-25">' +
        ...
        $('.row-bill-to').find('.customer-details').remove(),
          $('.row-bill-to').find('.col-bill-to').append(t)
      }),
      p.on('select2:open', function () {
        $(document).find('.add-new-customer').length ||
          $(document)
    ...
    s.length &&
      $(document).on('click', '.btn-apply-changes', function (s) {
        var c = $(this)
        if (
          ((o = c.closest('.dropdown-menu').find('#tax-1-input')),
          (l = c.closest('.dropdown-menu').find('#tax-2-input')),
          (a = c.closest('.dropdown-menu').find('#discount-input')),
          (t = c.closest('.repeater-wrapper').find('.tax-1')),
          (n = c.closest('.repeater-wrapper').find('.tax-2')),
          (e = $('.discount')),
          null !== o.val() && g(o, t),
          null !== l.val() && g(l, n),
          a.val().length)
    ...
    $(document).on('change', '.item-details', function () {
      ...
      e.next('textarea').length
        ...
        : e.after(
            '<textarea class="form-control mt-2" rows="2">' + t + '</textarea>'
          )
    }),
    m.length &&
      m.on('click', function () {
